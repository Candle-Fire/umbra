...
namespace rx {
    ...
    protected:
        ...
        struct CommandQueue {
            ...
            /**
             * Signal to the passed semaphore that the queue has finished.
             */
            void Signal(VkSemaphore sem);
            /**
             * Wait for the given semaphore to be signalled before this queue is processed.
             */
            void Wait(VkSemaphore sem);
        ...
        std::vector<VkSemaphore> signalPool;
        std::mutex signalLock;

        /**
         * Create or fetch a valid Semaphore from the signalling pool.
         * Most invocations will create a new semaphore.
         * Return a semaphore to the pool using FreeSemaphore.
         * @return a valid VkSemaphore
         */
        VkSemaphore FetchSemaphore() {
            std::scoped_lock lock(signalLock);
            if (signalPool.empty()) {
                VkSemaphore& sem = signalPool.emplace_back();
                VkSemaphoreCreateInfo create = { VK_STRUCTURE_TYPE_SEMAPHORE_CREATE_INFO };
                assert(vkCreateSemaphore(device, &create, nullptr, &sem) == VK_SUCCESS);
            }

            VkSemaphore sem = signalPool.back();
            signalPool.pop_back();
            return sem;
        }
        ...
        struct VulkanThreadCommands {
            ...
            std::vector<std::pair<QueueType, VkSemaphore>> waitQueues;                            // Semaphores that link the ThreadCommands to wait on a given queue.
            std::vector<VkSemaphore> waits;                                                       // Semaphores that this ThreadCommands is waiting on.
