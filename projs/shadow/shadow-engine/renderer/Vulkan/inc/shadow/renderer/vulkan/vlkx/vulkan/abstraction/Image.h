...
namespace vlkx {
    ...
    class Image {
      public:
        ...
        static ImageDescriptor loadCubeFromDisk(const std::string &directory,
    ...
    class ImageSampler {
      public:
        struct Config {
            explicit Config(VkFilter filter = VK_FILTER_LINEAR,
                            VkSamplerAddressMode mode = VK_SAMPLER_ADDRESS_MODE_REPEAT) : filter(filter), mode(mode) {}
    ...
    class RefCountedTexture : public SamplableImage {
      public:
        ...
        RefCountedTexture(const ImageLocation &location,
                          std::vector<ImageUsage> usages,
                          const ImageSampler::Config &config)
      ...
      private:
        ...
        static ReferenceCounter get(const ImageLocation &location,
                                    const std::vector<ImageUsage> &usages,
                                    const ImageSampler::Config &config);
