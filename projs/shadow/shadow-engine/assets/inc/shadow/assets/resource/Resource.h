...
namespace SH::Asset {
    ...
    struct ResourceType {
        ...
        static ResourceType INVALID_RESOURCE;
    };
...
namespace SH {
    ...
    template <>
    struct HashFunc<Asset::ResourceType> {
        static uint32_t get(const Asset::ResourceType& key) { return HashFunc<HeapHash>::get(key.hash); }
    };
}
...
#pragma pack(1)
    struct ResourceHeader {
        ...
        uint32_t magic = MAGIC; // SER Format header
        uint32_t version = 0;
        uint32_t flags = 0;
        uint32_t padding = 0;
