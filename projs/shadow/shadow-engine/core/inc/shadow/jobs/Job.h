...
template<typename T>
constexpr T align(T v, T a) { return ((v + a - T(1)) / a) * a; }
...
namespace SH::Jobs {
    ...
    struct TaskGroup {
        ...
        inline uint32_t execute() {
            ...
            args.group = group;
            if (shared > 0) {
                args.data = alloca(shared + 64);
                args.data = (void*) align((size_t) args.data, (size_t) 64);
            } else
                args.data = nullptr;
            ...
            return context->count.fetch_sub(1);
        }
    };
    ...
    struct PriorityQueue {
        ...
        std::mutex wakeLocker;                      //
        std::condition_variable wait;               // The condition that will unblock any wait()ing threads
        std::mutex waitLocker;                      //
        uint8_t threadLookup[256] = {};             // Give each active thread a byte address, here's the lookup table

        inline uint8_t nextIndex() {
            return threadLookup[nextQueue.fetch_add(1, std::memory_order_relaxed)];
        }

        inline TaskQueue& nextTaskQueue() {
        ...
        inline void execute(size_t firstQueue) {
            ...
            for (size_t i = 0; i < nThreads; i++) {
                TaskQueue& tasks = queues[threadLookup[firstQueue]];
                while (tasks.pop(task)) {
                    uint32_t ctr = task.execute();
                    if (ctr == 1) {
                        std::unique_lock<std::mutex> lock(waitLocker);
                        wait.notify_all();
                    }
                }
                firstQueue++;
    ...
    /**
     * Long-running jobs need a way to know if they're about to be shut down so they can close more gracefully.
     * @return whether the Job System module is about to close.
     */
    bool isShuttingDown();
    ...
    /**
     * Run the given task off-thread and immediately return control.
